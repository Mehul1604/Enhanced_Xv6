...
main(int argc, char **argv)
{
  ...
  40:	e8 c6 02 00 00       	call   30b <kill>
  for(i=1; i<argc; i++)
 ...
 199:	31 f6                	xor    %esi,%esi
{
 ...
 1b8:	50                   	push   %eax
 1b9:	6a 00                	push   $0x0
 1bb:	e8 33 01 00 00       	call   2f3 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 221:	e8 f5 00 00 00       	call   31b <open>
  if(fd < 0)
 ...
 235:	50                   	push   %eax
 236:	e8 f8 00 00 00       	call   333 <fstat>
  close(fd);
 23b:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 23e:	89 c6                	mov    %eax,%esi
  close(fd);
 240:	e8 be 00 00 00       	call   303 <close>
  return r;
 ...
 39c:	89 d1                	mov    %edx,%ecx
{
 ...
 40e:	56                   	push   %esi
 40f:	57                   	push   %edi
 410:	e8 e6 fe ff ff       	call   2fb <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
 ...
 489:	e8 6d fe ff ff       	call   2fb <write>
 ...
 4e9:	e8 0d fe ff ff       	call   2fb <write>
        putc(fd, c);
      } else {
 ...
 4fd:	e8 f9 fd ff ff       	call   2fb <write>
  for(i = 0; fmt[i]; i++){
 ...
 571:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 574:	e8 82 fd ff ff       	call   2fb <write>
        ap++;
 ...
 5eb:	57                   	push   %edi
 5ec:	53                   	push   %ebx
 5ed:	e8 09 fd ff ff       	call   2fb <write>
        while(*s != 0){
