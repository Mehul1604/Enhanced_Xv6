 ...
 149:	31 f6                	xor    %esi,%esi
{
 ...
 168:	50                   	push   %eax
 169:	6a 00                	push   $0x0
 16b:	e8 33 01 00 00       	call   2a3 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 1d1:	e8 f5 00 00 00       	call   2cb <open>
  if(fd < 0)
 ...
 1e5:	50                   	push   %eax
 1e6:	e8 f8 00 00 00       	call   2e3 <fstat>
  close(fd);
 1eb:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 1ee:	89 c6                	mov    %eax,%esi
  close(fd);
 1f0:	e8 be 00 00 00       	call   2b3 <close>
  return r;
 ...
 34c:	89 d1                	mov    %edx,%ecx
{
 ...
 3be:	56                   	push   %esi
 3bf:	57                   	push   %edi
 3c0:	e8 e6 fe ff ff       	call   2ab <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
 ...
 439:	e8 6d fe ff ff       	call   2ab <write>
 ...
 499:	e8 0d fe ff ff       	call   2ab <write>
        putc(fd, c);
      } else {
 ...
 4ad:	e8 f9 fd ff ff       	call   2ab <write>
  for(i = 0; fmt[i]; i++){
 ...
 521:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 524:	e8 82 fd ff ff       	call   2ab <write>
        ap++;
 ...
 59b:	57                   	push   %edi
 59c:	53                   	push   %ebx
 59d:	e8 09 fd ff ff       	call   2ab <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
 ...
 6bb:	e8 53 fc ff ff       	call   313 <sbrk>
  if(p == (char*)-1)
