...
main(void)
{
  ...
  while((fd = open("console", O_RDWR)) >= 0){
      ...
      33:	e8 a3 0d 00 00       	call   ddb <open>
...
fork1(void)
{
  ...
  exit();
      ...
      if(chdir(buf+3) < 0)
      bd:	e8 49 0d 00 00       	call   e0b <chdir>
      ...
      e3:	50                   	push   %eax
      e4:	e8 da 0c 00 00       	call   dc3 <close>
      break;
...
00000180 <runcmd>:
{
  ...
  switch(cmd->type){
    ...
    if(pipe(p) < 0)
     ...
     1ab:	50                   	push   %eax
     1ac:	e8 fa 0b 00 00       	call   dab <pipe>
  ...
  if(pid == -1)
    ...
    close(p[0]);
     ...
     1ee:	e8 d0 0b 00 00       	call   dc3 <close>
    close(p[1]);
     1f3:	58                   	pop    %eax
     1f4:	ff 75 f4             	pushl  -0xc(%ebp)
     1f7:	e8 c7 0b 00 00       	call   dc3 <close>
    wait();
  ...
  if(pid == -1)
    ...
    exec(ecmd->argv[0], ecmd->argv);
     ...
     235:	52                   	push   %edx
     236:	50                   	push   %eax
     237:	e8 97 0b 00 00       	call   dd3 <exec>
  ...
  if(pid == -1)
    ...
    close(rcmd->fd);
     ...
     27b:	e8 43 0b 00 00       	call   dc3 <close>
    if(open(rcmd->file, rcmd->mode) < 0){
     280:	58                   	pop    %eax
     281:	5a                   	pop    %edx
     282:	ff 73 10             	pushl  0x10(%ebx)
     285:	ff 73 08             	pushl  0x8(%ebx)
     288:	e8 4e 0b 00 00       	call   ddb <open>
    ...
    panic("runcmd");
     ...
     2d1:	6a 01                	push   $0x1
     2d3:	e8 eb 0a 00 00       	call   dc3 <close>
      dup(p[1]);
     2d8:	58                   	pop    %eax
     2d9:	ff 75 f4             	pushl  -0xc(%ebp)
     2dc:	e8 32 0b 00 00       	call   e13 <dup>
      close(p[0]);
     2e1:	58                   	pop    %eax
     2e2:	ff 75 f0             	pushl  -0x10(%ebp)
     2e5:	e8 d9 0a 00 00       	call   dc3 <close>
      close(p[1]);
     2ea:	58                   	pop    %eax
     2eb:	ff 75 f4             	pushl  -0xc(%ebp)
     2ee:	e8 d0 0a 00 00       	call   dc3 <close>
      runcmd(pcmd->left);
     2f3:	5a                   	pop    %edx
     ...
     2ff:	6a 00                	push   $0x0
     301:	e8 bd 0a 00 00       	call   dc3 <close>
      dup(p[0]);
     306:	5a                   	pop    %edx
     307:	ff 75 f0             	pushl  -0x10(%ebp)
     30a:	e8 04 0b 00 00       	call   e13 <dup>
      close(p[0]);
     30f:	59                   	pop    %ecx
     310:	ff 75 f0             	pushl  -0x10(%ebp)
     313:	e8 ab 0a 00 00       	call   dc3 <close>
      close(p[1]);
     318:	58                   	pop    %eax
     319:	ff 75 f4             	pushl  -0xc(%ebp)
     31c:	e8 a2 0a 00 00       	call   dc3 <close>
      runcmd(pcmd->right);
     321:	58                   	pop    %eax
     ...
     c59:	31 f6                	xor    %esi,%esi
{
  ...
  for(i=0; i+1 < max; ){
    ...
    cc = read(0, &c, 1);
     ...
     c78:	50                   	push   %eax
     c79:	6a 00                	push   $0x0
     c7b:	e8 33 01 00 00       	call   db3 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
  ...
  fd = open(n, O_RDONLY);
     ...
     ce1:	e8 f5 00 00 00       	call   ddb <open>
  if(fd < 0)
  ...
  r = fstat(fd, st);
     ...
     cf5:	50                   	push   %eax
     cf6:	e8 f8 00 00 00       	call   df3 <fstat>
  close(fd);
     cfb:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
     cfe:	89 c6                	mov    %eax,%esi
  close(fd);
     d00:	e8 be 00 00 00       	call   dc3 <close>
  return r;
...
00000da3 <waitx>:
SYSCALL(waitx)
     da3:	b8 16 00 00 00       	mov    $0x16,%eax
     da8:	cd 40                	int    $0x40
     daa:	c3                   	ret    

00000dab <pipe>:
SYSCALL(pipe)
     dab:	b8 04 00 00 00       	mov    $0x4,%eax
     db0:	cd 40                	int    $0x40
     db2:	c3                   	ret    

00000db3 <read>:
SYSCALL(read)
     db3:	b8 05 00 00 00       	mov    $0x5,%eax
     db8:	cd 40                	int    $0x40
     dba:	c3                   	ret    

00000dbb <write>:
SYSCALL(write)
     dbb:	b8 10 00 00 00       	mov    $0x10,%eax
     dc0:	cd 40                	int    $0x40
     dc2:	c3                   	ret    

00000dc3 <close>:
SYSCALL(close)
     dc3:	b8 15 00 00 00       	mov    $0x15,%eax
     dc8:	cd 40                	int    $0x40
     dca:	c3                   	ret    

00000dcb <kill>:
SYSCALL(kill)
     dcb:	b8 06 00 00 00       	mov    $0x6,%eax
     dd0:	cd 40                	int    $0x40
     dd2:	c3                   	ret    

00000dd3 <exec>:
SYSCALL(exec)
     dd3:	b8 07 00 00 00       	mov    $0x7,%eax
     dd8:	cd 40                	int    $0x40
     dda:	c3                   	ret    

00000ddb <open>:
SYSCALL(open)
     ddb:	b8 0f 00 00 00       	mov    $0xf,%eax
     de0:	cd 40                	int    $0x40
     de2:	c3                   	ret    

00000de3 <mknod>:
SYSCALL(mknod)
     de3:	b8 11 00 00 00       	mov    $0x11,%eax
     de8:	cd 40                	int    $0x40
     dea:	c3                   	ret    

00000deb <unlink>:
SYSCALL(unlink)
     deb:	b8 12 00 00 00       	mov    $0x12,%eax
     df0:	cd 40                	int    $0x40
     df2:	c3                   	ret    

00000df3 <fstat>:
SYSCALL(fstat)
     df3:	b8 08 00 00 00       	mov    $0x8,%eax
     df8:	cd 40                	int    $0x40
     dfa:	c3                   	ret    

00000dfb <link>:
SYSCALL(link)
     dfb:	b8 13 00 00 00       	mov    $0x13,%eax
     e00:	cd 40                	int    $0x40
     e02:	c3                   	ret    

00000e03 <mkdir>:
SYSCALL(mkdir)
     e03:	b8 14 00 00 00       	mov    $0x14,%eax
     e08:	cd 40                	int    $0x40
     e0a:	c3                   	ret    

00000e0b <chdir>:
SYSCALL(chdir)
     e0b:	b8 09 00 00 00       	mov    $0x9,%eax
     e10:	cd 40                	int    $0x40
     e12:	c3                   	ret    

00000e13 <dup>:
SYSCALL(dup)
     e13:	b8 0a 00 00 00       	mov    $0xa,%eax
     e18:	cd 40                	int    $0x40
     e1a:	c3                   	ret    

00000e1b <getpid>:
SYSCALL(getpid)
     e1b:	b8 0b 00 00 00       	mov    $0xb,%eax
     e20:	cd 40                	int    $0x40
     e22:	c3                   	ret    

00000e23 <sbrk>:
SYSCALL(sbrk)
     e23:	b8 0c 00 00 00       	mov    $0xc,%eax
     e28:	cd 40                	int    $0x40
     e2a:	c3                   	ret    

00000e2b <sleep>:
SYSCALL(sleep)
     e2b:	b8 0d 00 00 00       	mov    $0xd,%eax
     e30:	cd 40                	int    $0x40
     e32:	c3                   	ret    

00000e33 <uptime>:
SYSCALL(uptime)
     e33:	b8 0e 00 00 00       	mov    $0xe,%eax
     e38:	cd 40                	int    $0x40
     e3a:	c3                   	ret    

00000e3b <set_priority>:
     ...
     e5c:	89 d1                	mov    %edx,%ecx
{
  ...
  write(fd, &c, 1);
     ...
     ece:	56                   	push   %esi
     ecf:	57                   	push   %edi
     ed0:	e8 e6 fe ff ff       	call   dbb <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
  ...
  write(fd, &c, 1);
     ...
     f49:	e8 6d fe ff ff       	call   dbb <write>
  ...
  write(fd, &c, 1);
     ...
     fa9:	e8 0d fe ff ff       	call   dbb <write>
        putc(fd, c);
      } else {
  ...
  write(fd, &c, 1);
     ...
     fbd:	e8 f9 fd ff ff       	call   dbb <write>
  for(i = 0; fmt[i]; i++){
  ...
  write(fd, &c, 1);
    1034:	e8 82 fd ff ff       	call   dbb <write>
        ap++;
  ...
  write(fd, &c, 1);
    ...
    10ab:	57                   	push   %edi
    10ac:	53                   	push   %ebx
    10ad:	e8 09 fd ff ff       	call   dbb <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
  ...
  p = sbrk(nu * sizeof(Header));
    ...
    11cb:	e8 53 fc ff ff       	call   e23 <sbrk>
  if(p == (char*)-1)
