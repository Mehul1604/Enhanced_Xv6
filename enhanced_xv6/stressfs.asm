  ...
  1f:	31 db                	xor    %ebx,%ebx
{
  ...
  a6:	50                   	push   %eax
  a7:	e8 3f 03 00 00       	call   3eb <open>
...
//    printf(fd, "%d\n", i);
  ...
  c0:	56                   	push   %esi
  c1:	57                   	push   %edi
  c2:	e8 04 03 00 00       	call   3cb <write>
  for(i = 0; i < 20; i++)
  ...
  d2:	57                   	push   %edi
  d3:	e8 fb 02 00 00       	call   3d3 <close>
  ...
  f5:	50                   	push   %eax
  f6:	e8 f0 02 00 00       	call   3eb <open>
 ...
 108:	56                   	push   %esi
 109:	57                   	push   %edi
 10a:	e8 b4 02 00 00       	call   3c3 <read>
  for (i = 0; i < 20; i++)
 ...
 11a:	57                   	push   %edi
 11b:	e8 b3 02 00 00       	call   3d3 <close>
 ...
 269:	31 f6                	xor    %esi,%esi
{
 ...
 288:	50                   	push   %eax
 289:	6a 00                	push   $0x0
 28b:	e8 33 01 00 00       	call   3c3 <read>
    if(cc < 1)
...
stat(const char *n, struct stat *st)
{
 ...
 2f1:	e8 f5 00 00 00       	call   3eb <open>
  if(fd < 0)
 ...
 305:	50                   	push   %eax
 306:	e8 f8 00 00 00       	call   403 <fstat>
  close(fd);
 30b:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 30e:	89 c6                	mov    %eax,%esi
  close(fd);
 310:	e8 be 00 00 00       	call   3d3 <close>
  return r;
 ...
 46c:	89 d1                	mov    %edx,%ecx
{
 ...
 4de:	56                   	push   %esi
 4df:	57                   	push   %edi
 4e0:	e8 e6 fe ff ff       	call   3cb <write>
  while(--i >= 0)
...
printf(int fd, const char *fmt, ...)
{
 ...
 559:	e8 6d fe ff ff       	call   3cb <write>
 ...
 5b9:	e8 0d fe ff ff       	call   3cb <write>
        putc(fd, c);
      } else {
 ...
 5cd:	e8 f9 fd ff ff       	call   3cb <write>
  for(i = 0; fmt[i]; i++){
 ...
 641:	88 45 e7             	mov    %al,-0x19(%ebp)
  write(fd, &c, 1);
 644:	e8 82 fd ff ff       	call   3cb <write>
        ap++;
 ...
 6bb:	57                   	push   %edi
 6bc:	53                   	push   %ebx
 6bd:	e8 09 fd ff ff       	call   3cb <write>
        while(*s != 0){
...
malloc(uint nbytes)
{
 ...
 7db:	e8 53 fc ff ff       	call   433 <sbrk>
  if(p == (char*)-1)
